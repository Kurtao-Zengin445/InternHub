...
@section('page-subtitle', $evaluation->internship->application->student->user->name)
...
<div class="col-xl-8">
    ...
    <div class="card mb-3" style="background:{{ $gradeBg }};border-color:transparent">
        <div class="card-body d-flex align-items-center gap-4" style="padding:28px">
            ...
            <div class="ms-auto text-end">
                ...
                <div style="font-size:12px;color:{{ $gradeColor }};opacity:.6;margin-top:2px">Pembimbing Perusahaan</div>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body" style="padding:24px">
            ...
            @php $val=$evaluation->{$comp['key']}??0; $bc=$val>=80?'#10b981':($val>=70?'#f59e0b':'#ef4444'); @endphp
            <div class="score-bar-wrap">
                <div class="score-bar-label">{{ $comp['label'] }} <span style="font-size:11px;color:#94a3b8">({{ $comp['weight'] }}%)</span></div>
            ...
            <div class="pt-3 border-top mt-1 d-flex justify-content-between align-items-center">
                ...
                <span style="font-size:20px;font-weight:800;color:{{ $gradeColor }}">{{ number_format($score,1) }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body" style="padding:24px">
            ...
            <div class="{{ $evaluation->improvements || $evaluation->notes ? 'mb-4' : '' }}">
            ...
            <div class="{{ $evaluation->strengths ? 'pt-4 border-top' : '' }} {{ $evaluation->notes ? 'mb-4' : '' }}">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('company.evaluations.edit', $evaluation) }}" class="btn btn-outline-primary">
...
<div class="col-xl-4 d-flex flex-column gap-3">
    @php $student = $evaluation->internship->application->student; @endphp
    ...
    <div class="card">
        ...
        <div class="card-body" style="font-size:13px;padding:16px 20px">
            <div class="d-flex align-items-center gap-3 mb-3">
                <div style="width:44px;height:44px;border-radius:11px;background:#ede9fe;color:#4c1d95;font-size:17px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0">
                    {{ strtoupper(substr($student->user->name, 0, 1)) }}
                </div>
                <div>
                    <div style="font-weight:700;color:#0f172a">{{ $student->user->name }}</div>
                    <div style="color:#94a3b8;font-size:12px">{{ $student->nis }} · {{ $student->class }}</div>
                </div>
            </div>
            <div class="d-flex justify-content-between mb-1">
                <span style="color:#94a3b8">Sekolah</span>
                <span style="font-weight:500;text-align:right;max-width:160px">{{ $student->school->name }}</span>
            </div>
            <div class="d-flex justify-content-between">
                <span style="color:#94a3b8">Program</span>
                <span style="font-weight:500;text-align:right;max-width:160px">{{ Str::limit($evaluation->internship->application->program->title, 24) }}</span>
            </div>
        </div>
    </div>

    {{-- Nilai sekolah --}}
    @php $supervisorEval = $evaluation->internship->supervisorEvaluation(); @endphp
    <div class="card">
        <div class="card-header"><i class="bi bi-arrow-left-right text-secondary me-2"></i>Nilai Sekolah</div>
        <div class="card-body text-center" style="padding:20px">
            @if($supervisorEval)
                <div style="font-size:36px;font-weight:800;color:#1a56db;letter-spacing:-1px">{{ number_format($supervisorEval->final_score,1) }}</div>
                <div style="font-size:14px;font-weight:700;color:#64748b;margin-top:2px">Grade {{ $supervisorEval->grade_letter }}</div>
            @else
                <div style="font-size:13px;color:#94a3b8">Belum dinilai oleh sekolah</div>
            @endif
        </div>
    </div>
    ...
    <div class="card" style="border-color:#bfdbfe;background:#eff6ff">
        <div class="card-body text-center" style="padding:20px">
            <div style="font-size:11px;color:#3b82f6;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px">Nilai Akhir Gabungan</div>
            <div style="font-size:40px;font-weight:800;color:#1e40af;letter-spacing:-1.5px">{{ number_format($finalScore,1) }}</div>
            <div style="font-size:12px;color:#3b82f6;margin-top:4px">40% Sekolah + 60% Perusahaan</div>
        </div>
    </div>
    @endif
</div>
